...
class ProcessScheduled extends Command
{
    ...
    public function handle(TenantContext $ctx): int
    {
        ...
        // Resume paused drips (quiet-hours / daily-cap reached). The job re-checks
        // its own limits and re-pauses if still not allowed, so nudging the paused
        // set every ~10 minutes is safe and cheap. Campaigns mid-run sit in
        // 'sending' and are left untouched.
        $resumed = 0;
        if ((int) $now->format('i') % 10 === 0) {
            foreach (Campaign::where('status', 'paused')->get() as $c) {
                SendCampaign::dispatch($c->tenant_id, $c->id);
                $resumed++;
            }
        }

        $this->info('Processed ' . $orders->count() . ' scheduled orders, dispatched ' . $due->count() . ' campaigns, resumed ' . $resumed . '.');
