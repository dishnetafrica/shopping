...
class OfferVision
{
    ...
    public function extract(string $base64Jpeg, string $caption = ''): ?array
    {
        ...
        $cacheKey = 'offerimg:' . $model . ':' . sha1($b64 . '|' . mb_strtolower(trim($caption)));
        $cached = Cache::get($cacheKey);
        if ($cached !== null) return $cached === 'NONE' ? null : $cached;

        $json = $this->callVision($b64, $caption, $model);
        $offer = is_array($json) ? OfferExtractor::fromVision($json) : null;
        if ($offer && empty($offer['found'])) $offer = null;

        Cache::put($cacheKey, $offer ?? 'NONE', now()->addDays(self::CACHE_DAYS));
        return $offer;
    }
...
{"found":true|false,"title":"<dish or offer name>","price":<integer or null>,"currency":"<UGX|SSP|KES|USD|INR or empty>","items":["<item>", ...],"day":"<weekday or 'today' or empty>","offer_type":"<{$types}>","description":"<one short line or empty>","confidence":<0-100>}
Rules:
