...
$r = E($DEF_A)->handle('Rice', $CAT_A, [], []);
ck('Rice -> Rice 5kg added', nameOf($r,'Rice')==='Pakistan Rice 5kg' && qtyOf($r,'Rice')===1);
...
$r = E($DEF_A)->handle('Rice and Sugar', $CAT_A, [], []);
ck('Rice and Sugar -> both added (5kg + 1kg)', nameOf($r,'Rice')==='Pakistan Rice 5kg' && nameOf($r,'Sugar')==='Kinyara Sugar 1kg' && lines($r)===2);
...
echo "\n-- MIXED SIZE + DEFAULT --\n";
$r = E($DEF_A)->handle('Rice and Sugar 2kg', $CAT_A, [], []);
ck('Rice and Sugar 2kg -> Rice 5kg default + Sugar 2kg', nameOf($r,'Rice')==='Pakistan Rice 5kg' && nameOf($r,'Sugar')==='Kinyara Sugar 2kg');

echo "\n-- OUT OF STOCK (Rice 5kg OOS) --\n";
$catOOS = $CAT_A; $catOOS[2]['stock'] = 0;          // Rice 5kg default out of stock
$r = E($DEF_A)->handle('Rice', $catOOS, [], []);
...
$ra = E($DEF_A)->handle('Rice', $CAT_A, [], []);
$rb = E($DEF_B)->handle('Rice', $CAT_B, [], []);
...
try { for ($i=0;$i<1000;$i++){ $res = E($DEF_A)->handle('Rice', $CAT_A, $cart, $state); $cart=$res['cart']; $state=$res['state']; } }
...
try {
  for ($i=0;$i<500;$i++){
    $a=E($DEF_A)->handle('Rice',$CAT_A,$cartA,$stA); $cartA=$a['cart']; $stA=$a['state'];
    $b=E($DEF_B)->handle('Rice',$CAT_B,$cartB,$stB); $cartB=$b['cart']; $stB=$b['state'];
  }
