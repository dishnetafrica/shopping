...
ck('A1 "Rice" -> Added default Rice 5kg', cartName($r,'Rice')==='Pakistan Rice 5kg' && cartQty($r,'Rice')===1);
...
ck('A3 "milk" -> Added default Fresh Dairy 1L', cartName($r,'Milk')==='Fresh Dairy Milk 1L');
...
$r1 = eng($DEF)->handle('rice', $CAT, [], []);
$hint1 = stripos($r1['reply'],'different size')!==false;
$r2 = eng($DEF)->handle('milk', $CAT, $r1['cart'], $r1['state']);   // thread state
...
ck('H2 strategy=explicit_then_auto, no default -> auto-picks cheapest (Local Rice 1kg)', cartName($r,'Rice')==='Local Rice 1kg');
